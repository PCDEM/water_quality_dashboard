...
wqShp <- sf::st_read('GIS/Waterbody_IDs_(WBIDs).shp') |>
  ...
  dplyr::mutate(
    ...
    color = ifelse(Type == 'MARINE' & Status == 'Active', '#00244c',
            ifelse(Type == 'MARINE' & Status == 'Inactive', '#6A5ACD',
  ...
  do <- data.frame(
    rd |>
      ...
      dplyr::summarise(
        ...
        n_below = ifelse(Type == 'MARINE', length(which(DOsat < 42)),
...
sci <- readxl::read_excel('Data/Pinellas_SCI.xlsx') |>
  ...
  dplyr::filter(WBID == '1604B1') |>
  tidyr::drop_na(Date, `SCI Score`) |>
  dplyr::group_by(Segment, WBID, Date) |>
  dplyr::summarise(`SCI Score` = mean(`SCI Score`, na.rm = TRUE)) 
...
dat <- readxl::read_excel('Data/Pinellas_RedTide_Entero.xlsx') |>
  dplyr::mutate(`Karenia brevis` = stringr::str_to_title(`Karenia brevis`),
    Color = ifelse(`Karenia brevis` < 1000, "#878686",
                 ifelse(`Karenia brevis` %in% seq(1000,10000), "white", 
                 ifelse(`Karenia brevis` %in% seq(10001,100000), "yellow",
                 ifelse(`Karenia brevis` %in% seq(100001,1000000), "orange",
                 ifelse(`Karenia brevis` %in% seq(1000001,10000000), "red","black"))))),

...
wqPath <- 'data/pinellas_wq_2003_2026.xlsx'
...
# cospRaw <- readxl::read_excel('data/cosp_data.xlsx', sheet = 'SWAMP')
# 
# # Reformat SP data to match PC data:
# cospWQ <- cospRaw |>
#   dplyr::filter(Analysis != 'Color (True)') |>
...
#          Analyte,
#          Result) |>
#   dplyr::filter(Analyte %in% c('Chlorophyll-a', 'Enterococci', 'Dissolved Oxygen',
#                         'Dissolved Oxygen Saturation', 'pH', 'Salinity',
#                         'Turbidity', 'Temperature', 'Nitrate+Nitrite-N',
#                         'Total Nitrogen', 'Orthophosphate as P', 'Total Phosphorous',
#                         'Total Kjeldahl Nitrogen', 'TSS', 'Secchi Disk', 'E. coli'),
#                 substr(Site,1,11) != 'Field Blank') |>
...
#   dplyr::rename(`DO%` = 'Dissolved Oxygen Saturation',
...
#          DO = 'Dissolved Oxygen',
#          `Chl-a` = 'Chlorophyll-a') |>
#   dplyr::mutate_at(c('Latitude', 'Longitude', 'DO', 'DO%', 'Salinity', 'Secchi',
#               'Turbidity','Temp_Water', 'NOX', 'TKN', 'OP','TP','TSS','E_coli',
#               'Enterococci','pH', 'Chl-a'), as.numeric) |>
...
# 
# 
# wqIn <- dplyr::bind_rows(wqIn, cospW) |>
#   dplyr::mutate(Type = ifelse(WBID == '1716A', 'TIDAL', Type))
...
wqShp <- sf::st_read('GIS/Waterbody_IDs_(WBIDs).shp') |>
  ...
  dplyr::mutate(
    ...
    Status = ifelse(WBID %in% c(
      #'1556A', '1530', '1603B', '1624A'
      '1558C', '1558B', '1709D', '1709F', '1716A','1696', 
...
streamShp <- sf::st_read('GIS/staceywbid.shp') |>
  sf::st_transform("+proj=longlat +ellps=WGS84 +datum=WGS84") |>
  sf::st_make_valid() |>
  dplyr::filter(
  BASIN_1 %in% c('ANCLOTE RIVER', 'KLOSTERMAN BAYOU RUN', 'CURLEW CREEK',
                 'STEVENSON CREEK', 'LAKE TARPON CANAL', 'MOCCASIN CREEK',
                 'BISHOP CREEK', 'MULLET CREEK', 'ALLIGATOR CREEK', 'ALLEN CREEK',
                 'LAKE SEMINOLE OUTLET', 'MCKAY CREEK', 'LONG BRANCH',
                 'CROSS CANAL (NORTH)', 'CROSS CANAL (SOUTH)', 'CHURCH CREEK',
                 'PINELLAS PARK DITCH', 'BONN CREEK', 'PASADENA LAKE OUTLET',
                 'BEAR CREEK', 'BOOKER CREEK', 'CLAM BAYOU DRAIN',
                 '77TH AVE CANAL', 'CEDAR CREEK', 'SUTHERLAND BAYOU',
                 'COW BRANCH', 'BELLEAIR GOLFCLUB RUN')
)
...
Sinf <- function(site){
  ...
  if (Sw %in% c(1604, '1604B1','1604B2')) allens(input, 'info')
...
nsModal <- modalDialog(
  ...
  #HTML("<b>Water quality sampling in this location transitioned from Pinellas County to the City of St. Petersburg in 2013. Data collected prior to 2013 was reported by Pinellas County. For more information on water quality in this area please contact julie.vogel@stpete.org</b>"),
  easyClose = TRUE,
  footer = modalButton("Close")
)
...
observeEvent(input$mapplo_shape_click, {
  if (site() %in% c('SMACKS BAYOU', 'TAMPA BAY (UPPER SEGMENT)', 'TAMPA BAY (MIDDLE SEGMENT)', 'BEAR CREEK',
...
rcd <- reactive({
  ...
  rd <- wqIn |>
    ...
    dplyr::filter(Level == 'Surface') |>
    dplyr::select(Date, WBID, Type, `Chl-a`, `DO%`, TKN, NOX, TP, E_coli, 
                  Enterococci) |>
    dplyr::mutate(TN = TKN + NOX) |>
...
ap <- reactive({
    ...
    annDat <- wqIn |>
      ...
      dplyr::filter(WBID == wqShp$WBID[wqShp$ID == site()],
                    ...
                    Level == 'Surface') |>
      dplyr::select(WBID, Year, Date, TN, TP, `Chl-a`) |>
      dplyr::group_by(WBID, Year, Date) |>
      dplyr::summarise(dplyr::across(c(TN, TP, `Chl-a`), 
                                     \(x) mean(x, na.rm = TRUE))) |>
      dplyr::ungroup()


    if (unique(annDat$WBID) %in% c('1528A','1618C','1694B','1694A','1558N','1558I',
                                   '1558H2','1558H','1558G','1558F','1661')) {
      dat <- annDat |>
        dplyr::group_by(WBID, Year) |>
        dplyr::summarise(dplyr::across(c(TN, TP),
                                       \(x) psych::geometric.mean(x, na.rm = TRUE)),
                         `Chl-a` = mean(`Chl-a`, na.rm = TRUE)) |>
        dplyr::ungroup() |>
        dplyr::filter(Year < lubridate::year(Sys.Date())) |>
        dplyr::mutate(
          dplyr::across('TN':'Chl-a', round, 2),
          `Chl-a` = round(`Chl-a`, 1)
        )
    } else {
      dat <- annDat |>
 ...
 # display table
 fillCol(flex = c(1, rep(1,3)),
         plotly::renderPlotly(ap()$annTN),
         plotly::renderPlotly(ap()$annTP),
         plotly::renderPlotly(ap()$annChl)
 )
...
observeEvent(input$active_tab, {
  ...
  } else if(input$active_tab == 'Annual Summary'){
    output$rightPanel <- renderUI({
      fillCol(flex = c(1, 1),
      paste0('\nThese plots show the average annual values for total nitrogen,
            total phosphorus, and chlorophyll-a for the ', stringr::str_to_title(site()),
            ' watershed from ', lubridate::year(min(getData()$Date)),' to ',
            lubridate::year(max(getData()$Date)),'. These values are calculated
            using arithmetic mean or geometric mean depending on watershed specific
            criteria set by FDEP (refer to the Annual Report for more detail).
            The orange bars represent years when the average value was above
            the FDEP criteria (failing) and the green bars represent years when 
            the average values was below the FDEP criteria (passing).')
      )
    })
  } 
...
<!-- sci <- readxl::read_excel('data/Pinellas_SCI.xlsx', sheet = 'SCI Scores') |> -->
<!--   dplyr::mutate(Date = as.Date(Date), -->
<!--                 Bio = 'SCI', -->
<!--                 SCI = `SCI Score`, -->
<!--                 HA = `Habitat Score`, -->
...
<!--   dplyr::summarise(dplyr::across(c(SCI, HA, RPS), meanNA)) |> -->
<!--   dplyr::ungroup() |> -->
<!--   dplyr::mutate(Score = SCI) -->
...
<!--   dplyr::mutate_at(c('Score','SCI','HA','RPS'), function(x) ifelse(is.nan(x), NA, x)) -->
...
<!--   return( -->
<!--     taxaTbl( -->
<!--       x = dat |> dplyr::select(where(~ !all(is.na(.)))), -->
<!--       bio = ifelse(getBio()$bio$Bio[1] == 'SCI', 'SCI', 'LVI'), -->
<!--       type = 'metrics' -->
<!--       ) -->
<!--     ) -->
...
Pinellas County staff visit beaches along the county's Gulf coast throughout
the year to collect samples and determine if there is an active red tide bloom.
In addition, the [Florida FWC](https://myfwc.com/research/redtide/statewide/)
collects red tide samples throughout the state and
provide updates on current conditions and forecasting of events.
...
[FL Department of Health](https://www.floridahealthybeaches.com/counties) remains the official source for updates and advisories about 
...
observeEvent(input$prev_date1, {
  prev_date1 <- max(rdate$Date[rdate$Date < as.Date(input$rdate)], na.rm = TRUE)
...
observeEvent(input$next_date1, {
  next_date1 <- min(rdate$Date[rdate$Date > as.Date(input$rdate)], na.rm = TRUE)
...
# Create a sequence of dates not in the dataset for the date picker:
bd <- data.frame(
  Date = seq.Date(bdate$Date[1], bdate$Date[length(bdate$Date)], by = 'day')
  ) |>
  dplyr::filter(! Date %in% bdate$Date) |>
  dplyr::pull(Date)
...
observeEvent(input$prev_date, {
  prev_date <- max(bdate$Date[bdate$Date < as.Date(input$bdate)], na.rm = TRUE)
...
observeEvent(input$next_date, {
  next_date <- min(bdate$Date[bdate$Date > as.Date(input$bdate)], na.rm = TRUE)
...
This report is the most recent annual water quality prepared by the Pinellas County 
Division of Environmental Management and describes sampling methodology, data 
trends, water quality summary statistics, and biological monitoring results. For 
more information about this report or to request previous years' reports, please
contact Stacey Day (sday@pinellas.gov).
...
fillCol(flex = c(1,0.1),
  tags$iframe(style = "border: none; width: 100%; height: 100%;",
              src = "2024 PCDEM Ambient Water Quality Report.pdf"))
```
    ...
    out <- reactable::reactable(
      ...
      columns = list(
        Latitude      = reactable::colDef(
          footerStyle = list(fontWeight = "bold"),
          format      = reactable::colFormat(digits = 6, separators = FALSE),
          resizable   = TRUE
        ),
...
# Full period-of-record data download button:
...
# Download handler for selected site data. The sheets need to be added to the file
# or else only the data will be downloaded and not the units and qualifiers:

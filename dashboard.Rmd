...
#1. Change chla to arithmetic mean for E's
#2. Add table of averages to the summary tab
```
...
wqIn <- readxl::read_excel("Data/Pinellas_County_Water_Quality_Data_2003_2024.xlsx", guess_max = 50000)
...
# # details for site status legend:
# shapes <- c('rect', 'rect','rect')
# symbols <- setNames(Map(
#   f = leaflegend::makeSymbol,
#   shape = shapes,
#   fillColor = 'white',
#   color = c('green','red','darkgray'),
#   opacity = 1,
#   fillOpacity = 0.5,
#   height = 20,
#   width = 20,
...
m <- m |> leaflet::addPolygons(
  ...
  #  images = symbols,
  #   labels = c('Active', 'Inactive','Not Sampled'),
  #   labelStyle = "font-size: 14px;",
  #   width = 15,
  #   height = 13,
  #   orientation = 'vertical',
  #   title = htmltools::tags$div('Site Status',
...
#### This Dashboard
This dashboard is designed to make the Division’s (PCDEM) monitoring results accessible and summarize how the lakes, streams, and coastal waters measure up compared to past conditions and based on regulatory and management targets. Click on the tabs "Data Selection" drop down menu above to explore the different data types and learn more about our water quality and other habitat monitoring data.
...
<!-- 1) [__Stream Flow__](#stream-flow): USGS gage stations that monitor stream flow, water level, and precipitation. -->
<!-- 1) [__Stream Index__](#stream-index): Stream condition index data from various streams in Pinellas County. -->
1) [__Red Tide__](#red-tide): Current Pinellas County red tide data.
<!-- 1) [__Seagrass__](#seagrass): Seagrass survey data from Pinellas County coastal waters (in development). -->
...
txt <- modalDialog(
  ...
  HTML("<em>NOTE: This dashboard is still under development and some features may not be fully functional or respond as expected. Please report any issues to almanos@pinellas.gov.</em>"),
  tags$p(""),
  br(),
  ...
  br(),
  HTML("<b>Downloading the data:</b>"),
  tags$p(""),
  tags$li("Click the 'Data Downloads' tab at the top of the page to view raw data."),
  tags$p(""),
  # tags$li("Click the 'Water Quality' tab to view and search the raw data."),
  # tags$p(""),
  tags$li("Click the 'Download Data' button to download a copy of the data."),
  easyClose = TRUE,
  footer = NULL
)
...
#### Navigating the Map
Use the cursor to hover over the sites on the Map tab and click any to view water
quality data for that site. Refer the legend on the bottom left of the map which
denotes both the type of water body and the status of monitoring activity within 
that area. There are coastal, lake, stream, and not sampled sites, of which may
active (current monitoring activities) or inactive (previous monitoring conducted,
but none currently).
...
#### Report Card
The Report Card tab displays as "Pass/Fail" classification for selected parameters
...
#### Annual Report
Click the annual report tab view and download our most recent annual water quality 
report. This document is a detailed description of water quality sampling program
including sampling design, methods, data analysis, and results. 
...
# sdv <- purrr::partial(sd, na.rm = TRUE)
# minN <- purrr::partial(min, na.rm = TRUE)
# maxN <- purrr::partial(max, na.rm = TRUE)
# 
# sumStats <- reactive({
#   
#   x <- data.frame(wqM |>
#   dplyr::filter(WBID == 1440))
#   
#   # Make a table of summary statistics for each parameter (DO, chl_a, TN,TP,TSS,Turbidity,
# # Secchi, Temp, Salinity, Ecoli, and Entero) and put it in a table:
# p <- x |>
#   dplyr::select(DO, chl=chl_a, TN, TP, TSS, Turbidity, Secchi, Temp, Salinity, Ecoli, Entero) |>
#   dplyr::summarise(
#     dplyr::across(
#       dplyr::everything(), list(n = ~sum(!is.na(.)),`Min.`=minN,`Max.` = maxN,
#                                 mean = mean_n, 
#                                 geom = psych::geometric.mean,
#                                       median = med, stddev = sdv))) |>
#   round(2) |>
#   tidyr::pivot_longer(cols = everything(), names_to = "key", values_to = "value") %>%
#   tidyr::separate(key, into = c("param", "stat"), sep = "_") %>%
#   tidyr::pivot_wider(names_from = "stat", values_from = "value") |>
#   dplyr::rename(Parameter = param, Mean = mean, `Geometric Mean` = geom,
#          Median = median, `Std. Deviation` = stddev)
# 
# plotly::plot_ly() |>
#   plotly::add_trace(type = "table",
#             header = list(values = c("Parameter", "n","Min.","Max.","Mean", "Geometric Mean",
#                                      "Median","Std. Deviation"),
#                           align = c("center", "center"),
#                           fill = list(color = "#BCBCBC")),
#             cells = list(values = list(p$Parameter,p$n,p$Min.,p$Max.,p$Mean, p$`Geometric Mean`, 
#                                        p$Median, p$`Std. Deviation`),
#                          align = c("center", "center"),
#                          fill = list(color = "white", color = c("grey")),
#                          height=20,
#                          font = list(size=12)))
# })
      ...
      fillCol(flex = c(1, rep(1,5)),
        HTML('<em>This tab is under development. The Pass/Fail status for each parameter in a watershed is provisional and needs further verification.</em>'),
          paste('\nThis water quality report card for the',stringr::str_to_title(site()),
                'watershed is based on criteria set by FDEP with a Pass/Fail status for each parameter determined by annual threshold exceedence of 10% of samples (DO) or geometric mean (all other parameters).'),
        paste0(''),
        paste0(''),
        paste0(''))
    })
  } else if(input$active_tab == 'Annual Report'){
    output$gaugePlots <- renderUI({
      fillCol(flex = c(1, rep(1,5)),
...
<!-- ```{r USGS, include=FALSE} -->

<!-- # USGS stream gage sites in Pinellas County: -->
<!-- sites <- c('02307323','02307359','02307359','02307362','02307445','02309447', -->
<!--            '02307496','02307498','02309425','02309421','02309415','02307668', -->
<!--            '02307669','02307674','02309200','02309110','02307780','02308859', -->
<!--            '02308861','02308870','02308889','02308950','02310075') -->

<!-- # Parameters to retrieve from USGS: -->
<!-- # 00045 = Total Precipitation (in) -->
<!-- # 00060 = Stream Flow (ft^3/s) -->
<!-- # 00065 = Gage height (ft) -->
<!-- codes <- c('00045', '00060', '00065') -->

<!-- # Get info on data available at each site: -->
<!-- Udat <- dataRetrieval::whatNWISdata(siteNumber = sites) |> -->
<!--   dplyr::filter(data_type_cd != 'qw', parm_cd %in% codes) -->
...
<!-- # Start by creating the base map with tiles and layers control options -->
<!-- Um <- leaflet::leaflet(options = leaflet::leafletOptions(zoomControl = TRUE, -->
<!--                   zoomSnap = 0.1, zoomDelta = 1, attributionControl = FALSE)) |> -->
<!--   leaflet::addTiles() |> -->
<!--   leaflet::addProviderTiles(leaflet::providers$Esri.WorldImagery, group = 'Esri.WorldImagery') |> -->
<!--   leaflet::addProviderTiles(leaflet::providers$CartoDB.Positron, group = 'CartoDB.Positron') |> -->
<!--   leaflet::addProviderTiles(leaflet::providers$CartoDB.DarkMatter, group = 'CartoDB.DarkMatter') |> -->
...
<!--   leaflet::addLayersControl( -->
<!--   position = "topleft", -->
<!--   baseGroups = c("Esri.WorldImagery","CartoDB.Positron", "CartoDB.DarkMatter", -->
<!--                  "Esri.WorldTopoMap", "OpenTopoMap")) -->
...
<!-- Uplo <- leaflet::leafletProxy('Uplo') -->


<!-- observeEvent(input$Uplo_marker_click, { -->
<!--     siteLoc <- Udat %>% dplyr::filter(dec_lat_va == input$Uplo_marker_click$lat & -->
<!--                        dec_long_va == input$Uplo_marker_click$lng) -->
<!--   Uplo <- Uplo %>% -->
<!--     leaflet::removeMarker('s') %>% -->
<!--     leaflet::addCircleMarkers( -->
<!--       data = siteLoc, -->
<!--       lng = ~dec_long_va, -->
<!--       lat = ~dec_lat_va, -->
...
<!-- output$Uplo <- leaflet::renderLeaflet(Um) -->
<!-- leaflet::leafletOutput('Uplo') -->

<!-- ``` -->



<!-- ## Column {.tabset .tabset-fade data-width="840"} -->

<!-- ### Discharge, Gage Height, & Precipitation -->

<!-- ```{r} -->

<!-- # Get available USGS data for selected site: -->
<!-- gage <- reactive({ -->
<!--   if (is.null(input$Uplo_marker_click)){ -->
<!--     getDat <- Udat |> -->
<!--       dplyr::filter(site_no == Uinit) -->
<!--     } else { -->
<!--     getDat <- Udat |> -->
<!--       dplyr::filter(dec_lat_va == input$Uplo_marker_click$lat & -->
<!--              dec_long_va == input$Uplo_marker_click$lng) -->
<!--     } -->
<!--    return(getDat) -->
<!-- }) -->
...
<!-- # Plot for stream flow: -->
<!-- output$flow <- plotly::renderPlotly({ -->
<!--   g <- Usite(gage,'00060','00003') -->
<!--   Uplt(data = g$g, stnm = g$f$station_nm, title = 'Daily Streamflow', -->
<!--        yax = 'Discharge (cubic feet per second)') -->
<!-- }) -->

<!-- # flow <- reactive({ -->
<!-- #   g <- Usite(gage,'00060','00003') -->
<!-- #   return(Uplt(data = g$g, stnm = g$f$station_nm, title = 'Daily Streamflow', -->
<!-- #        yax = 'Discharge (cubic feet per second)')) -->
<!-- # }) -->

<!-- # Plot for gage height: -->
<!-- output$gh <- plotly::renderPlotly({ -->
<!--   g <- Usite(gage,'00065','00003') -->
<!--   Uplt(data = g$g, stnm = g$f$station_nm, title = 'Daily Gage Height', -->
<!--        yax = 'Gage Height (ft)') -->
<!-- }) -->

<!-- # Plot for precipitation: -->
<!-- output$rain <- plotly::renderPlotly({ -->
<!--   g <- Usite(gage,'00045','00006') -->
<!--   Uplt(data = g$g, stnm = g$f$station_nm, title = 'Daily Precipitation', -->
<!--        yax = 'Precipitation (in)') -->
<!-- }) -->

<!-- # Render plots: -->
<!-- fillCol(flex = c(1,rep(1,3)), -->
<!--         plotly::plotlyOutput('flow'), -->
<!--         #pltmsg(flow,'X_00060_00003',flow()), -->
<!--         plotly::plotlyOutput('gh'), -->
<!--         plotly::plotlyOutput('rain')) -->
...
<!--   output$ts_3d <- plotly::renderPlotly({ -->

<!--    f <- gage() |> dplyr::filter(parm_cd == '00060', stat_cd == '00003') -->
<!--    g <- dataRetrieval::readNWISdv(siteNumbers = f$site_no, -->
<!--                          parameterCd = '00060', -->
<!--                          statCd = '00003', -->
<!--                          startDate = f$begin_date, -->
...
<!--     FlowMatrix       <- data.frame(Day = lubridate::yday(g$Date), -->
<!--                                    Year = lubridate::year(g$Date), -->
<!--                                    Var = g[,4]) -->
<!--     varMat           <- raster::as.matrix(raster::rasterFromXYZ(FlowMatrix)) -->
...
<!--     tckMo            <- month.abb[lubridate::parse_date_time(tckNum, orders = "j") |> -->
<!--                                     lubridate::month()] -->

<!--     legTitle <- 'Flow (cfs)' -->

<!--     plotly::plot_ly(height = 900) |> -->
<!--       plotly::add_surface(z = varMat, -->
<!--                   y = y, -->
...
<!--                   showscale = TRUE) |> -->
<!--       plotly::layout(title = list(text = dataRetrieval::readNWISsite(g$site_no)$station_nm,x = 0.47, y = 0.92), -->
<!--              scene=list( -->
<!--                yaxis = list(title = 'Year', dtick = ifelse(length(unique(FlowMatrix$Year)) <= 10,2,5)), -->
<!--                zaxis = list(title = legTitle), -->
<!--                xaxis = list(title = 'Month', autorange = "reversed", -->
<!--                             tickvals = seq(1, 366, by = 30.5), -->
<!--                             ticktext = tckMo), -->
<!--                camera = list(eye = list(x = 1.3, y = 1.3, z = 1.8)))) -->
<!--   }) -->

<!-- plotly::plotlyOutput('ts_3d') -->
...
<!-- radioButtons('type','Select data type',choices=c('Flow','Gage Height','Precipitation'), -->
<!--              selected='Flow',inline=TRUE,width='95%') -->


<!--   # Monthly boxplots: -->
<!--   output$month_box <- plotly::renderPlotly({ -->
<!--       f <- gage() |> dplyr::filter(parm_cd == '00060', stat_cd == '00003') -->
<!--       dat <- dataRetrieval::readNWISdv(siteNumbers = f$site_no, -->
<!--                          parameterCd = '00060', -->
<!--                          statCd = '00003', -->
<!--                          startDate = f$begin_date, -->
<!--                          endDate = f$end_date) -->
<!--     dat$Month <- factor(month.abb[lubridate::month(dat$Date)], levels = month.abb) -->
<!--     plotly::plot_ly(data = dat, x = ~Month, y = ~X_00060_00003, type = 'box', -->
<!--             marker = list(color = '#4ABBE6'), -->
<!--             line = list(color = '#B2B2B2')) |> -->
<!--       plotly::layout(title = paste('Monthly Distribution of Flow at', -->
<!--                                    stringr::str_to_title(f$station_nm)), -->
<!--              xaxis = list(linecolor = 'black',width = 0.5, mirror = TRUE, -->
<!--                           title = ''), -->
<!--              yaxis = list(linecolor = 'black',width = 0.5, mirror = TRUE, -->
<!--                           title = 'Flow (cfs)',rangemode = 'tozero')) -->
<!--   }) -->

<!--   # Month time series: -->
<!--   output$month_ts <- plotly::renderPlotly({ -->
<!--       f <- gage() |> dplyr::filter(parm_cd == '00060', stat_cd == '00003') -->
<!--       dat <- dataRetrieval::readNWISdv(siteNumbers = f$site_no, -->
<!--                          parameterCd = '00060', -->
<!--                          statCd = '00003', -->
...
<!--     datSum <- data.frame(dat |> -->
<!--                            dplyr::group_by(month,year) |> -->
<!--                            dplyr::summarise_at(.vars = 4, .funs = mean)) -->
<!--     plotly::plot_ly(data = datSum, x = ~year,y = ~X_00060_00003, color = ~month, -->
<!--                     type = 'scatter', #line = list(shape='spline'), -->
<!--             mode = 'lines', colors = viridis::viridis_pal(option = "D")(12)) |> -->
<!--       plotly::layout(title = paste('Average Monthly Flow at', stringr::str_to_title(f$station_nm)), -->
<!--              xaxis = list(linecolor = 'black',width = 0.5, mirror = TRUE, -->
<!--                           title = ''), -->
<!--              yaxis = list(linecolor = 'black',width = 0.5, mirror = TRUE, -->
<!--                           title = 'Flow (cfs)',rangemode = 'tozero'), -->
<!--              legend = list(plot_bgcolor = 'blue')) -->
<!--   }) -->


<!-- #     # Day of year time series: -->
<!-- # output$doy_ts <- plotly::renderPlotly({ -->
<!-- #   f <- gage() |> dplyr::filter(parm_cd == '00045', stat_cd == '00006') -->
<!-- #   dat <- dataRetrieval::readNWISdv(siteNumbers = f$site_no, -->
<!-- #                          parameterCd = '00045', -->
<!-- #                          statCd = '00006', -->
<!-- #                          startDate = f$begin_date, -->
...
<!-- #     # -->
<!-- #     plotly::plot_ly(dat, x = ~doy, y = ~X_00045_00006, type = 'scatter', mode = 'lines', -->
<!-- #                   line = list(color = '#76D0E1')) -->
<!-- #   }) -->



<!-- fillCol(flex = c(1,1,1,1), -->
<!--         plotly::plotlyOutput('month_box'), -->
<!--         plotly::plotlyOutput('month_ts')) -->
<!--         #plotly::plotlyOutput('doy_ts')) -->
...
<!-- ```{r, echo = FALSE, out.width = '28%', fig.align = 'center', fig.alt='Stream Index page under construction'} -->
<!-- knitr::include_graphics('www/sci.png') -->
<!-- ``` -->
...
<!-- ### FWC Red Tide Map -->
<!-- # ```{r, echo = FALSE, out.width = '28%', fig.align = 'center', fig.alt='Red tide page under construction'} -->
<!-- # knitr::include_graphics('www/rtpg.png') -->
<!-- # ``` -->
<!-- #  -->
<!-- # Visit the [red tide map](https://www.arcgis.com/apps/webappviewer/index.html?id=7c18e5858be14368a660bda35f4acc68){target="_blank"} for the latest counts of red tide in and around Pinellas County. -->
<!-- #  -->
<!-- <iframe width="98%" height="98%" src="http://myfwc.maps.arcgis.com/apps/View/index.html?appid=87162eec3eb846218cec711d16462a72"> -->
<!-- </iframe> -->

<iframe width="99%" height="98%" src="https://www.arcgis.com/apps/webappviewer/index.html?id=7c18e5858be14368a660bda35f4acc68">
</iframe>

<!-- <iframe width="100%" height="600" src="http://myfwc.maps.arcgis.com/apps/View/index.html?appid=87162eec3eb846218cec711d16462a72"> -->
<!-- </iframe> -->
...
<!-- ```{r, echo = FALSE, out.width = '28%', fig.align = 'center', fig.alt='Seagrass page under construction'} -->
<!-- knitr::include_graphics('www/sgpg.png') -->
<!-- ``` -->

<!-- Visit the Tampa Bay Estuary Program [seagrass dashboard](https://shiny.tbep.org/seagrasstransect-dash/){target="_blank"} for information on seagrass transect data in Tampa Bay. -->
...
output$wqdata <- downloadHandler(
  #filename = function(){'PCDEM_WQ_Data.csv'},
  filename = function(){'PCDEM_WQ_Data.xlsx'},

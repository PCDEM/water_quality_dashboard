...
wqPath <- 'data/pinellas_wq_2003_2025.xlsx'
...
ap <- reactive({
    ...
    annDat <- wqIn |>
      ...
      dplyr::filter(WBID == wqShp$WBID[wqShp$ID == site()],
                    lubridate::year(Date) != lubride::year(Sys.Date())) |>
      dplyr::select(Site, Year, WBID, Date, TN, TP, `DO%`, `Chl-a`) |>
      dplyr::group_by(Site, Year, WBID, Date) |>
      dplyr::summarise(dplyr::across(c(TN, TP, `DO%`, `Chl-a`), \(x) mean(x, na.rm = TRUE))) |>
      dplyr::ungroup() |>
      dplyr::mutate(Site = dplyr::case_when(
        substr(Site, 1, 1) %in% c('E','W','L','S','R'), substr(Site,1,2), Site
        )) |>
      dplyr::group_by(Site, Year, WBID, Date) |>
      dplyr::summarise(dplyr::across(c(TN, TP, `DO%`, `Chl-a`), \(x) mean(x, na.rm = TRUE))) |>
      dplyr::ungroup()
    ...
    datArith <- annDat |>
      dplyr::filter(Site %in% c('W4','W5','W6','W7','W8','E1','E2','E3','E4','E5','RB')) |>
      dplyr::group_by(Site, WBID, Year) |>
      dplyr::summarise(dplyr::across(c(TN, TP, `DO%`), 
                                     \(x) psych::geometric.mean(c, na.rm = TRUE)),
                       `Chl-a` = mean(`Chl-a`, na.rm = TRUE)) |>
      dplyr::ungroup() |>
      dplyr::group_by(Site) |>
      dplyr::filter(max(Year) == lubridate::year(Sys.Date()) - 1) |>
      dplyr::ungroup()

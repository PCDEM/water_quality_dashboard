...
wqInit <- readxl::read_excel("Data/Pinellas_County_Water_Quality_Data_2003_2024.xlsx",
                           guess_max = 50000) |>
...
rcd <- reactive({
  ...
  if (! unique(rd$WBID) %in% c('1558I','1558H','1558H2','1558G','1558F', 1661,'1528A',
                   '1558N','1694A','1694B','1618C','1558B','1558C')) {
  repo <- rd |>
    dplyr::select(-`DO%`) |>
...
<!--   list( -->
<!--     bio = dplyr::filter(bioData, WBID == wbid), -->
<!--     taxa = dplyr::filter(sciTaxa, WBID == wbid), -->
<!--     met  = dplyr::filter(sciMet, WBID == wbid) -->
<!--   ) -->
...
<!-- taxaT <- reactive({ -->
<!--   return(taxaTbl(getBio()$taxa, type = 'counts')) -->
<!-- }) -->

<!-- metT <- reactive({ -->
<!--   return(taxaTbl(getBio()$met, type = 'metrics')) -->
<!-- }) -->
...
<!--     getBio()$bio$Bio[1] == 'SCI' & score > 40 & score <= 65 ~ "MARGINAL", -->
<!--     getBio()$bio$Bio[1] == 'SCI' & score > 65 ~ "HEALTHY", -->
<!--     getBio()$bio$Bio[1] == 'LVI' & score <= 42 ~ "IMPAIRED", -->
<!--     getBio()$bio$Bio[1] == 'LVI' & score > 42 ~ "HEALTHY" -->
<!--   ) -->
...
<!--   score <- round(meanNA(getBio()$bio$Score), 0) -->
...
dat <- readxl::read_excel('Data/Pinellas_RedTide_Entero.xlsx', guess_max = 50000) |>
  dplyr::mutate(
    `Karenia brevis` = stringr::str_to_title(`Karenia brevis`),
...
output$datSel <- downloadHandler(
  ...
  content = function(file){
    ...
    units <- readxl::read_excel('Data/Pinellas_County_Water_Quality_Data_2003_2024.xlsx',
                          sheet = 'Units')
    quals <- readxl::read_excel('Data/Pinellas_County_Water_Quality_Data_2003_2024.xlsx',
                          sheet = 'Qualifier Codes')

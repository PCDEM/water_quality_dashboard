  ...
  rd <- data.frame(
    wqIn |>
      ...
      dplyr::select(Date, WBID, Type, Level, chl_a = `Chl-a`, chla_q = `Chla_q`,
                    DOsat = `DO%`, DOsat_q = `DO%_q`, TKN, TKN_q, NOX, NOX_q, 
...
output$chlg <- highcharter::renderHighchart({
  gauge(data = dplyr::select(getData(), Date, chl_a), var = 'Chlorophyll-a',unit=' µg/L',
        ...
        round=1, width = input$screen_width,
...
output$TNg <- highcharter::renderHighchart({
  gauge(data = dplyr::select(getData(), Date, TN), var = 'Total Nitrogen',unit=' mg/L',
        ...
        round=2, width = input$screen_width,
...
output$TSSg <- highcharter::renderHighchart({
  gauge(data = dplyr::select(getData(), Date, TSS), var = 'Total Suspended Solids',unit=' mg/L',
        max = 16,g1=0,g2=16,round=2, width = input$screen_width , r1=0,r2=0)
})
...
output$Turg <- highcharter::renderHighchart({
  gauge(data = dplyr::select(getData(), Date, Turbidity), var = 'Turbidity',unit=' NTU',
        max = 20,g1=0,g2=20,round=2, r1=0, r2=0, width = input$screen_width )
})
...
output$Secg <- highcharter::renderHighchart({
  gauge(data = dplyr::select(getData(), Date, Secchi), var = 'Secchi Depth',unit=' m',
        max = 4,g1=0,g2=4,round=2,r1=0,r2=0, width = input$screen_width )
})
...
output$Tg <- highcharter::renderHighchart({
  gauge(data = dplyr::select(getData(), Date, Temp), var = 'Temperature',unit='°C',
        max = 30,g1=0,g2=30,round=2,r1=0,r2=0, width = input$screen_width )
})
...
output$eg <- highcharter::renderHighchart({
  gauge(data = dplyr::select(getData(), Date, Ecoli), var = HTML('<em>E. coli</em>'),unit=' MPN/100mL',
        ...
        round=0, width = input$screen_width,
...
While red tide blooms can occur year-round, they are most common in Florida around late summer to early fall. The blooms are driven by a combination of warm water temperatures, nutrient availability, and favorable currents. During red tide events, it’s best to limit beach activities, as the airborne toxins can cause eye, nose, and throat irritation. 
...
*Enterococcus* levels are regularly monitored to ensure beach safety, as it can indicate fecal pollution in the water. High levels often result from heavy rainfall or stormwater discharge, which can wash contaminants into coastal waters. When *Enterococcus* is elevated, it’s advisable to avoid swimming, particularly for people with open wounds or weakened immune systems. While the bacteria itself is not always harmful, it acts as a warning sign of possible health risks in recreational waters.
...
rDat <- dat |>
  dplyr::filter(Date == Date[max(which(!is.na(dat$`Karenia brevis`)))])

eDat <- dat |>
  dplyr::filter(Date == Date[max(which(!is.na(dat$Enterococcus)))])
...
leg2 <- setNames(
  Map(
    ...
    shape          = ll,
    color          = 'black',
    fillColor      = c('#878686', 'white', 'yellow', 'orange', 'red'),
    opacity        = 1,
    fillOpacity    = 0.8,
    height         = 15,
    width          = 20,
...
rtm <- leaflet::leaflet(options = leaflet::leafletOptions(zoomControl=FALSE)) |>
  ...
  leaflet::addScaleBar(
    ...
    leaflet::addControl(
    html = paste("<div style='font-size: 20px; color: red; font-weight: bold;'>Most recent sample:",unique(rDat$Date),"</div>"),
    position = "topleft"
  ) 
...
bm <- leaflet::leaflet(options = leaflet::leafletOptions(zoomControl=FALSE)) |>
  ...
  leaflet::addScaleBar(
    ...
    leaflet::addControl(
    html = paste("<div style='font-size: 20px; color: red; font-weight: bold;'>Most recent sample:",unique(eDat$Date),"</div>"),
    position = "topleft"
  ) 
...
<!-- ```{r} -->
<!-- # Initialize a reactive value to track if a water quality subtab has been clicked yet: -->
<!-- click2 <- reactiveVal(TRUE) -->

<!-- # UI for the initial informational popup: -->
<!-- Rtxt <- modalDialog( -->
<!--   title = "NAVIGATING THE RED TIDE DATA PAGE", -->
<!--   HTML("<i>*Note: if the red tide map does not appear to be working properly, please try refreshing the page.</i>"), -->
<!--   tags$p(""), -->
<!--   HTML("<b>Getting started:</b>"), -->
<!--   tags$p(""), -->
<!--   tags$li("View the points on the map to see the most recent red tide status."), -->
<!--   tags$p(""), -->
<!--   tags$li("The diamond points are sites monitored by Pinellas County."), -->
<!--   tags$p(""), -->
<!--   tags$li("The circle points are sites monitored by FWC."), -->
<!--   br(), -->
<!--   HTML("<b>Viewing the data:</b>"), -->
<!--   tags$p(""), -->
<!--   tags$li("Click any of the points to view more information about that site."), -->
<!--   tags$p(""), -->
<!--   tags$li("Click the popup arrow at the bottom of the map to view a table of the data."), -->
<!--   easyClose = TRUE, -->
<!--   footer = modalButton("Close"), -->
<!--   style = "font-size:90%" -->
<!-- ) -->
...
<!-- observeEvent(input$active_tab, { -->
<!--   if (input$active_tab == "Red Tide" & click2()) { -->
<!--     showModal(Rtxt) -->
<!--     click2(FALSE)  -->
<!--   } -->
<!-- }) -->
...
fillCol(flex = c(0.06,0.05, 1),
  ...
  shinyWidgets::downloadBttn('datSel', 'Download Selected Site Data', 
                             style = 'simple', block = TRUE, color = 'success'),
  # shinyWidgets::downloadBttn('datSel', paste0('Download Selected Site Data (',
                                              # format(object.size(wqIn), units = "KB"),')'), 
                             # style = 'simple', block = TRUE, color = 'success'),
